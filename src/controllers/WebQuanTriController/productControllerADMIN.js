...
import productSercive from "../../services/webQuanTriService/productServiceADMIN";
import multer from 'multer';
import path from 'path';
//import  uploadCloud from '../src/config/uploadFile'
...
let handleGetAllCategoryProduct = async (req, res) => {
    try {  
        let id = req.query.id;
        console.log(id)
        let data = await productSercive.handleGetAllCategoriesProductService(id);
     return res.status(200).json(data)
 } catch (error) {
     console.log("Lỗi phân quyền",error)
    return res.status(200).json({
...
let handleSearchProduct = async (req, res) => {
    try {  
        ...
        let data = await productSercive.handleSearchProductServiceADMIN(key_search,page);
     return res.status(200).json(data)
 } catch (error) {
     console.log("Lỗi phân quyền",error)
    return res.status(200).json({
...
module.exports = {
    ...
    handleGetAllCategoryProduct:handleGetAllCategoryProduct,
